...
def submit(messages: list, horizon_days: int = 7) -> str:
    ...
    try:
        conn.execute(
            "INSERT INTO chat_jobs (id, status, question, messages_json) VALUES (?, 'queued', ?, ?)",
            [job_id, question, json.dumps(messages)[:50000]],
        )
        conn.commit()
    finally:
        conn.close()
    _executor.submit(_run, job_id, messages, horizon_days)
    return job_id
...
def resume_interrupted():
    ...
    try:
        rows = conn.execute(
            "SELECT id, messages_json FROM chat_jobs WHERE status IN ('queued','running')").fetchall()
        pending = [(r["id"], r["messages_json"]) for r in rows]
    finally:
        conn.close()
    for jid, msgs in pending:
        try:
        ...
        _executor.submit(_run, jid, messages, 7)
    if pending:
